...
def get_ids(active_region=False):
	...
	if active_region == False:
		index_object = index_pages.objects.filter(emptied=False).all()[0]
		active_region = index_object.index_code
	else: 
		index_object = index_pages.objects.get(index_code=active_region)
	current_page = index_object.next_page 
	index_object.next_page += 1
	index_object.save()
	page_to_index = "https://www.qcc.com{}_{}".format(index_object.index_code, str(current_page))
	print(page_to_index)
	...
	if error_understood == False:		
		...
		print("Retrieved IDs for region code {}, page {}".format(index_object.index_code, current_page))
	return([company_id_list, active_region, current_page])
